...
extern String lidarData;
extern String lidarStrength;
extern String lidarTemperature;
...
// Add a LIDAR handler function
static esp_err_t lidar_handler(httpd_req_t *req) {
  // Prepare JSON response
  DynamicJsonDocument doc(1024);
  doc["distance"] = lidarData;  // Use the existing shared variable for distance
  doc["strength"] = lidarStrength; // Add a new variable for strength
  doc["temperature"] = lidarTemperature; // Add a new variable for temperature
...
// Add a new handler for LIDAR webpage
static esp_err_t lidar_page_handler(httpd_req_t *req) {
    // HTML with JavaScript for auto-refresh
            ...
            function fetchLidarData() {
                fetch('/lidar')
                    ...
                    .then(data => {
                        ...
                        document.getElementById("strength").innerText = data.strength || "N/A";
                        document.getElementById("temperature").innerText = data.temperature || "N/A";
                    })
                    .catch(err => console.error('Error fetching LIDAR data:', err));
            }

            // Refresh data every second
            setInterval(fetchLidarData, 1000);

            // Fetch data when the page loads
            window.onload = fetchLidarData;
        </script>
    </head>
    <body>
        <h1>LIDAR Real-Time Data</h1>
...
// LIDAR URI
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &lidar_page_uri);
    ...
    httpd_register_uri_handler(camera_httpd, &lidar_uri);
  }
